...
def client_task(client, server_url):
    """
    ...
    """
    try:
        # Step 1: Each client fetches the global model from the server
        ...
        update_response = requests.post(f"{server_url}/send_update", json=update_data)
        if update_response.status_code == 200:
            print(f"Client {client.client_id} successfully sent update to server.")
        else:
        ...
        agg_response = requests.get(f"{server_url}/aggregate")
