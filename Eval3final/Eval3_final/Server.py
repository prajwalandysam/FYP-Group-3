...
class Server:
    ...
    def add_node(self, model_update, metadata, client):
        ...
        # Connect the new node to the global model (index 0)
        self.adj_matrix[0][node_index] = 1

        # # Establish edges with relevant leaf nodes based on geographical proximity
        # for i, node in enumerate(self.nodes[:-1]):  # Exclude the last node (self)
        ...
        #     if distance <= 50 and self.adj_matrix[node_index][i] == 0:  # Check threshold and existing connection
        #         self.update_edge(node_index, i)
        #         self.update_edge(i, node_index)

        print("Adjacency Matrix after adding node:")
        self.print_dag()
